...
const SOLUTIONS = landings.map((l) => ({ name: l.navLabel, href: l.path, internal: true }));
    ...
    <nav aria-label={title}>
      ...
      <ul className="mt-4 space-y-2.5">
        {links.map((l) => {
          ...
          const external = !isMail && !l.internal;
          return (
            <li key={l.name}>
              <a
                href={l.href}
              ...
              >
                {l.name}
                {external && <ArrowUpRight size={11} className="opacity-50" />}
              </a>
            </li>
          );
        })}
